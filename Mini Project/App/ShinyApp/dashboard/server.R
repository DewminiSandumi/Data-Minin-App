...
dataset1<-read.csv("C:\\SLIIT\\3rdYear\\2ndSemester\\FDM\\Assignment\\dataset1.csv", header = T)

dataset<-read.csv("C:\\SLIIT\\3rdYear\\2ndSemester\\FDM\\Assignment\\dataset.csv", header = T)
names(dataset)
...
dataset3<-read.csv("C:\\SLIIT\\3rdYear\\2ndSemester\\FDM\\Assignment\\dataset3.csv", header = T)
...
#function(input, output) { 
  ...
  output$plot<-renderPlot({
    clusplot(dataset_5(), clusters()$cluster, main='2D representation of the Cluster solution', color = TRUE,shade=TRUE,labels=2, lines=0)
  })
